"""Main window for tr_works_scenario_ui — loads ui/scenario_ui.ui and wires
...
from tr_works_scenario_ui.managed_process import ManagedProcess
from tr_works_scenario_ui.scenario_action_client import ScenarioRosBridge


HW_LAUNCH_CMD = ["ros2", "launch", "tr_works_bringup", "hardware.launch.py"]
PLAYER_RUN_CMD = ["ros2", "run", "tr_works_scenario_player", "scenario_player_node.py"]


def _find_scenarios_dir() -> str:
    """Walk up from this file looking for a Scenarios/ directory."""
    ...
    for _ in range(10):
        ...
        if os.path.isdir(candidate):
            return candidate
    return os.path.join(os.path.expanduser("~"), "Project",
                        "TR-Works-Dev", "kkw", "TR-Works_ros2_ws", "Scenarios")
...
class ScenarioMainWindow(QMainWindow):
    ...
    def _stop_all(self) -> None:
        ans = QMessageBox.question(
            self, "Stop All",
            "Run /home/tc/kill_all_ros2.sh to terminate ALL ROS2 processes (incl. RViz, joint_control_ui, etc.)?",
        )
        if ans != QMessageBox.Yes:
            return
        try:
            subprocess.run(
                ["/home/tc/kill_all_ros2.sh"], timeout=15.0, check=False,
                stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL,
            )
        except Exception as e:
            self._log(f"kill_all_ros2.sh failed: {e}")
        self._hw.stop()
        self._player.stop()
