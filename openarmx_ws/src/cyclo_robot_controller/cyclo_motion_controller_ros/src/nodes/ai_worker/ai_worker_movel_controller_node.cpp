  ...
  left_gripper_position_(0.0)
{
  ...
  right_movel_sub_ = this->create_subscription<openarmx_scenario_player_msgs::msg::MoveL>(
    right_movel_topic_, 10,
    std::bind(&AIWorkerMoveLController::rightMoveLCallback, this, std::placeholders::_1));
  left_movel_sub_ = this->create_subscription<openarmx_scenario_player_msgs::msg::MoveL>(
...
void AIWorkerMoveLController::rightMoveLCallback(
  const openarmx_scenario_player_msgs::msg::MoveL::SharedPtr msg)
{
  ...
  right_movel_goal_pose_ = pose_utils::poseMsgToEigen(msg->pose);
...
void AIWorkerMoveLController::leftMoveLCallback(const openarmx_scenario_player_msgs::msg::MoveL::SharedPtr msg)
{
  if (!msg || !joint_state_received_ || jointStateTimedOut() || !q_desired_initialized_) {
    return;
  }
  ...
  left_movel_goal_pose_ = pose_utils::poseMsgToEigen(msg->pose);
...
void AIWorkerMoveLController::controlLoopCallback()
{
  ...
  try {
    ...
    if (right_movel_trajectory_active_ && right_elapsed < right_active_motion_duration_) {
      ...
      right_desired_vel =
        pose_utils::computeDesiredVelocity(
          right_gripper_pose_, pose_ref, kp_position_, kp_orientation_, linear_ref, angular_ref);
    } else {
      if (right_movel_trajectory_active_) {
        right_movel_trajectory_active_ = false;
      }
      // Trajectory elapsed — stay quiescent (avoid kp*error oscillation).
    }
    ...
    if (left_movel_trajectory_active_ && left_elapsed < left_active_motion_duration_) {
      ...
      left_desired_vel =
        pose_utils::computeDesiredVelocity(
          left_gripper_pose_, pose_ref, kp_position_, kp_orientation_, linear_ref, angular_ref);
    } else {
    ...
    cyclo_motion_controller::common::Vector6d left_weight =
      cyclo_motion_controller::common::Vector6d::Ones();
    right_weight.head<3>().setConstant(weight_task_position_);
    right_weight.tail<3>().setConstant(weight_task_orientation_);
    left_weight.head<3>().setConstant(weight_task_position_);
    left_weight.tail<3>().setConstant(weight_task_orientation_);
    ...
    if (!qp_controller_->getOptJointVel(optimal_velocities)) {
      pose_utils::publishStringMsg(controller_error_pub_, "AI Worker MoveL Controller: QP solve failed");
      return;
    }
    ...
    const std::string err =
      std::string("AI Worker MoveL Controller loop error: ") + e.what();
    pose_utils::publishStringMsg(controller_error_pub_, err);
...
{
  pose_utils::publishPoseStamped(r_gripper_pose_pub_, "base_link", this->now(), r_gripper_pose);
  pose_utils::publishPoseStamped(l_gripper_pose_pub_, "base_link", this->now(), l_gripper_pose);
}
