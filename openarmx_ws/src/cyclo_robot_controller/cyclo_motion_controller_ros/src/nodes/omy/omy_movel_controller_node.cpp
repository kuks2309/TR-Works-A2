...
void OmyMoveLControllerNode::moveLCallback(const openarmx_scenario_player_msgs::msg::MoveL::SharedPtr msg)
{
    ...
    if (movel_trajectory_active_ && elapsed < active_motion_duration_) {
      ...
      desired_task_vel =
        pose_utils::computeDesiredVelocity(
          current_pose, pose_ref, kp_position_, kp_orientation_, linear_ref, angular_ref);
    } else {
      if (movel_trajectory_active_) {
        movel_trajectory_active_ = false;
      }
      // Trajectory time elapsed — motion is finished. Stop publishing.
      // The post-elapse kp*error pull caused perpetual ±0.01 rad oscillation at 100Hz.
      // Holding the last commanded pose is handled by the trajectory controller's internal state.
      return;
    }
