...
DRIVE layer (구동-프로세스). Vendored verbatim from
openarmx_scenario_ui/openarmx_scenario_ui/managed_process.py so this dedicated
ptp UI stays self-contained (no cross-package dependency on the scenario UI —
same separation policy as the cyclo/pilz/ptp backend packages). Keep changes in
sync with that source if the kill logic is updated there.
...
captured to /tmp/openarmx_ptp_ui_<name>.log.
"""
...
class ManagedProcess:
    def __init__(self, name: str) -> None:
        ...
        self._log_path = f"/tmp/openarmx_ptp_ui_{_sanitize(name)}.log"
        self._log_file = None
